...
ui = shinyUI(fluidPage(
  ...
  helpText("Select desired stock and analysis"),
  ...
  conditionalPanel(
    ...
    absolutePanel(
      ...
      selectizeInput('pred_interval_naive', 'Prediction Interval', c('99', '95', '90', '80', '70','60'), selected = '95')
...
server = shinyServer(function(input, output){
  output$my_plot = renderPlot({
    ...
    # # Mean ####
    # if(input$forecasting_method == 'Mean' && input$stock_name != 'Select stock'){
    #   my_ts = na.omit(getSymbols.yahoo(input$stock_name, auto.assign = F,
    #                            from = input$start_time, to = input$end_time)
    #   )
    #   my_ts = my_ts[,4]
    ...
    # Naive ####
    if(input$forecasting_method == 'Naive' && input$stock_name != 'Select stock'){
      my_ts = na.omit(getSymbols.yahoo(input$stock_name, auto.assign = F,
                               from = input$start_time, to = input$end_time)
      )
      my_ts = my_ts[,4]
    ...
    # Seasonal Naive ####
    if(input$forecasting_method == 'Seasonal Naive' && input$stock_name != 'Select stock'){
      my_ts = na.omit(getSymbols.yahoo(input$stock_name, auto.assign = F,
                               from = input$start_time, to = input$end_time)
      )
      my_ts = my_ts[,4]
    ...
    if(input$forecasting_method == 'Return Tendencies' && input$stock_name != 'Select stock'){
      ...
      par(mfrow=c(2,3))
        plot(myts2)
      kernel_ <- kernel_data(input$stock_name, 1, -0.15, 0.15)
        plot(kernel_, main = paste("Density of the Returns of ", input$stock_name))
        range_1 <- kernel_$x < 0; range_2 <- kernel_$x > 0
        polygon(kernel_, border="black", lwd = 3)
        polygon(c(kernel_$x[range_1],0), c(kernel_$y[range_1],0), col="#ff8566", border="#ffcccc", lwd = 0.1)
        polygon(c(0,kernel_$x[range_2]), c(0,kernel_$y[range_2]), col="#8dd88d", border="#d9f2d9", lwd = 0.1)
        par(usr = c(0, 1, 0, 1))
        #text(0.15,0.5,paste(length(kernel_$x[which(kernel_$x <= -0.3)]-1)," smaller\n observations\n ←"))
        #text(0.85,0.5,paste(length(kernel_$x[which(kernel_$x >= 0.3)]-1)," larger\n observations\n →"))
